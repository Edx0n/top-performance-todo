...
Implementation (.NET C# with OpenTelemetry or Prometheus):
```csharp
// OpenTelemetry metrics (or use prometheus-net for Prometheus-native)
...
private static readonly Counter<long> OrdersTotal = Meter.CreateCounter<long>(
    "orders_total", "Total number of orders", unit: "orders");
private static readonly Histogram<double> OrderValue = Meter.CreateHistogram<double>(
    "order_value_dollars", "Order value in dollars", unit: "USD");

// In code
OrdersTotal.Add(1, new KeyValuePair<string, object?>("status", "completed"),
    new KeyValuePair<string, object?>("payment_method", "credit_card"));
OrderValue.Record(order.TotalAmount, new KeyValuePair<string, object?>("status", "completed"));
```
...
1. Latency:
   ...
   // C#: Request duration histogram
   var requestDuration = Meter.CreateHistogram<double>(
       "http_request_duration_seconds", "HTTP request duration", unit: "s");
...
2. Traffic:
   ...
   var requestsTotal = Meter.CreateCounter<long>(
       "http_requests_total", "Total HTTP requests");
   requestsTotal.Add(1, new KeyValuePair<string, object?>("method", method),
       new KeyValuePair<string, object?>("endpoint", endpoint),
       new KeyValuePair<string, object?>("status", statusCode));
...
3. Errors:
   ...
   var errorsTotal = Meter.CreateCounter<long>("errors_total", "Total errors");
   errorsTotal.Add(1, new KeyValuePair<string, object?>("service", serviceName),
...
4. Saturation:
   ...
   var connectionPoolUsage = Meter.CreateObservableGauge(
       "db_connection_pool_active", () => GetActiveConnectionCount(), "connections");
```
...
Implementation (ASP.NET Core middleware and logging):
```csharp
...
// Enrich logs with correlation ID (e.g. Serilog)
LogContext.PushProperty("CorrelationId", context.TraceIdentifier);
// Or with IHttpContextAccessor in a Serilog enricher:
enricher = (logEvent, propertyFactory) =>
    logEvent.AddProperty(propertyFactory.CreateProperty("CorrelationId",
        _httpContextAccessor.HttpContext?.TraceIdentifier ?? (object)""));
```
...
**Implementation (OpenTelemetry .NET):**
```csharp
...
async Task<Guid> CreateOrderAsync(OrderData orderData, CancellationToken ct)
{
    using var activity = ActivitySource.StartActivity("create_order");
    activity?.SetTag("order.items_count", orderData.Items.Count);
    activity?.SetTag("order.total", orderData.Total);

    using (ActivitySource.StartActivity("db.insert_order"))
    {
        orderId = await _db.InsertOrderAsync(orderData, ct);
    }

    using (var paymentActivity = ActivitySource.StartActivity("http.payment_service"))
    {
        paymentActivity?.SetTag("http.url", $"{_paymentOptions.Url}/payments");
        result = await _paymentService.ChargeAsync(orderId, orderData.Total, ct);
    }
...
Implementation (.NET OpenTelemetry):
```csharp
// Sample 1% of traces
builder.Services.AddOpenTelemetry()
    .WithTracing(tracing =>
    {
        tracing.SetSampler(new ParentBasedSampler(new TraceIdRatioBasedSampler(0.01)));
        // ...
    });
...
public class CustomSampler : Sampler
{
    ...
    {
        if (parameters.Tags.TryGetValue("http.status_code", out var status) &&
            status is int code && code >= 500)
            return new SamplingResult(SamplingDecision.RecordAndSample);

        if (parameters.Tags.TryGetValue("duration_ms", out var duration) &&
            duration is double ms && ms > 5000)
            return new SamplingResult(SamplingDecision.RecordAndSample);

        return new ParentBasedSampler(new TraceIdRatioBasedSampler(0.01))
            .ShouldSample(parameters);
    }
}
```
