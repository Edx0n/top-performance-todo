...
**Implementation Example (.NET C# with Polly):**
```csharp
// Circuit breaker policy with fallback
var circuitBreakerPolicy = Policy<HttpResponseMessage>
    ...
    .CircuitBreakerAsync(
        ...
        onBreak: (outcome, duration) => _logger.LogWarning("Circuit open for {Duration}s", duration.TotalSeconds),
...
var fallbackPolicy = Policy<HttpResponseMessage>
    ...
    .FallbackAsync(
        fallbackAction: (ct) => Task.FromResult(CreateFallbackResponse()),
        onFallbackAsync: (outcome) =>
        {
            _logger.LogError(outcome.Exception, "Payment service unavailable, using fallback");
            return Task.CompletedTask;
        }
    );

var result = await fallbackPolicy.WrapAsync(circuitBreakerPolicy)
    .ExecuteAsync(async () =>
    {
        using var client = _httpClientFactory.CreateClient("PaymentService");
        return await client.PostAsJsonAsync("/payments", new { orderId, amount });
    });

// Fallback response for graceful degradation
static HttpResponseMessage CreateFallbackResponse() =>
...
Implementation (.NET C# with Polly):
```
...
var retryPolicy = Policy<HttpResponseMessage>
    ...
    .WaitAndRetryAsync(
        ...
        onRetryAsync: (outcome, span, attempt, ctx) =>
        {
            _logger.LogWarning("Retry {Attempt} after {Delay}ms", attempt, span.TotalMilliseconds);
            return Task.CompletedTask;
        }
    );

var result = await retryPolicy.ExecuteAsync(() => _httpClient.GetAsync(requestUri));
```
...
var key = Request.Headers[IdempotencyKeyHeader].FirstOrDefault();
if (string.IsNullOrEmpty(key))
    return BadRequest("Idempotency-Key required");

var cached = await _cache.GetAsync<IActionResult>($"idem:{key}");
if (cached != null)
    return cached;

var result = await ProcessPaymentAndReturnResult();
await _cache.SetAsync($"idem:{key}", result, TimeSpan.FromHours(24));
return result;
```
...
**Implementation (.NET C# with Polly Bulkhead):**
```csharp
// Isolate concurrent calls per dependency
var paymentBulkhead = Policy.BulkheadAsync<HttpResponseMessage>(
    maxParallelization: 20,
    maxQueuingActions: 50,
    onBulkheadRejectedAsync: ctx =>
    {
        _logger.LogWarning("Bulkhead full for payment service");
        return Task.CompletedTask;
    });

var inventoryBulkhead = Policy.BulkheadAsync<HttpResponseMessage>(20, 50);
var notificationBulkhead = Policy.BulkheadAsync<HttpResponseMessage>(10, 20);

// Usage: each dependency has its own bulkhead
var paymentResult = await paymentBulkhead.ExecuteAsync(() => CallPaymentServiceAsync(data));
var inventoryResult = await inventoryBulkhead.ExecuteAsync(() => CallInventoryServiceAsync(data));

// Alternative with SemaphoreSlim for custom logic
private readonly SemaphoreSlim _paymentSemaphore = new(20, 20);

async Task<PaymentResult> CallPaymentServiceAsync(PaymentData data, CancellationToken ct)
{
    await _paymentSemaphore.WaitAsync(ct);
    try
    {
        return await _paymentService.ProcessAsync(data, ct);
    }
    finally
    {
...
```csharp
// HttpClient with connection timeout
...
```csharp
// Per-request or default timeout
...
```csharp
// Overall timeout with CancellationTokenSource
using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
var result = await CompleteCheckoutAsync(cts.Token);

// Or with Polly timeout policy wrapping the whole operation
var timeoutPolicy = Policy.TimeoutAsync(TimeSpan.FromSeconds(30));
var result = await timeoutPolicy.ExecuteAsync(ct => CompleteCheckoutAsync(ct), CancellationToken.None);
```
...
builder.Services.AddHealthChecks()
    ...
    .AddUrlGroup(new Uri(paymentServiceUrl + "/health/live"), name: "payment_service");

app.MapHealthChecks("/health/live", new HealthCheckOptions
{
    Predicate = _ => false  // Liveness: no checks, just that the process is up
});
...
**1. Cached Responses:**
```csharp
async Task<Recommendations> GetProductRecommendationsAsync(string userId, CancellationToken ct)
{
    try
    {
        return await _circuitBreaker.ExecuteAsync(() =>
            _mlService.GetRecommendationsAsync(userId, ct));
    }
    catch (BrokenCircuitException)
    {
        return await _cache.GetPopularProductsAsync(ct);
    }
}
```
...
**2. Default Values:**
```csharp
async Task<UserPreferences> GetUserPreferencesAsync(string userId, CancellationToken ct)
{
    try
    {
        return await _preferencesService.GetAsync(userId, ct);
    }
...
**3. Feature Toggles:**
```csharp
var recommendations = _featureManager.IsEnabledAsync("PersonalizedRecommendations").GetAwaiter().GetResult()
    ? await _mlService.GetRecommendationsAsync(ct)
    : await GetPopularProductsAsync(ct);
```
