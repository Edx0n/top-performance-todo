...
```csharp
async Task OnCustomerUpdatedAsync(CustomerUpdatedEvent eventData, CancellationToken ct)
{
    await _db.ExecuteAsync(
        "UPDATE orders SET customer_name = @Name, customer_email = @Email WHERE customer_id = @CustomerId",
        new { eventData.Name, eventData.Email, eventData.CustomerId },
...
Saga Orchestrator (.NET C#):
```csharp
var sagaState = new SagaState { SagaId = "saga-123", Status = SagaStatus.InProgress };

// Step 1
var result1 = await _accountService.DebitAsync(accountA, 100, sagaState.SagaId);
if (!result1.Success)
    return SagaResult.Fail("Insufficient funds");

sagaState.StepsCompleted.Add("debit_a");

// Step 2
var result2 = await _accountService.CreditAsync(accountB, 100, sagaState.SagaId);
if (!result2.Success)
{
    await _accountService.CreditAsync(accountA, 100, sagaState.SagaId); // Compensate
    return SagaResult.Fail("Account B invalid");
}
...
```csharp
async Task<DebitResult> DebitAccountAsync(Guid accountId, decimal amount, string sagaId, CancellationToken ct)
{
    var existing = await _db.QuerySingleOrDefaultAsync<Transaction>(
        "SELECT * FROM transactions WHERE saga_id = @SagaId AND operation = 'debit'",
        new { SagaId = sagaId }, cancellationToken: ct);
    if (existing != null)
        return new DebitResult { Success = true, TransactionId = existing.Id };

    var rowCount = await _db.ExecuteAsync(
        "UPDATE accounts SET balance = balance - @Amount WHERE id = @AccountId AND balance >= @Amount",
        new { Amount = amount, AccountId = accountId }, cancellationToken: ct);
    if (rowCount == 0)
        return new DebitResult { Success = false, Error = "Insufficient funds" };

    await _db.ExecuteAsync(
        "INSERT INTO transactions (saga_id, account_id, amount, operation) VALUES (@SagaId, @AccountId, @Amount, 'debit')",
        new { SagaId = sagaId, AccountId = accountId, Amount = amount }, cancellationToken: ct);
    return new DebitResult { Success = true };
}

async Task CompensateDebitAsync(Guid accountId, decimal amount, string sagaId, CancellationToken ct) =>
    await CreditAccountAsync(accountId, amount, sagaId, ct);
```
...
Event Handler (.NET C#):
```csharp
void HandleOrderPlaced(OrderPlacedEvent eventData)
{
    if (eventData.EventVersion == "1.0")
        ProcessOrderV1(eventData.Payload);
    else if (eventData.EventVersion == "2.0")
        ProcessOrderV2(eventData.Payload);
}
```
...
```csharp
OrderPlacedEvent UpcastEvent(OrderPlacedEvent eventData)
{
    if (eventData.EventType != "OrderPlaced" || eventData.EventVersion != "1.0")
        return eventData;

    return eventData with
    {
        EventVersion = "2.0",
        Payload = eventData.Payload with
        {
...
View Service (.NET C#):
```csharp
async Task OnOrderCreatedAsync(OrderCreatedEvent eventData, CancellationToken ct)
{
    await _db.ExecuteAsync(
        "INSERT INTO order_summary (order_id, customer_id, status, ...) VALUES (@OrderId, @CustomerId, @Status, ...)",
        new { eventData.OrderId, eventData.CustomerId, eventData.Status, ... },
        cancellationToken: ct);
}

async Task OnOrderShippedAsync(OrderShippedEvent eventData, CancellationToken ct)
{
    await _db.ExecuteAsync(
        "UPDATE order_summary SET status = 'shipped', last_updated = @Now WHERE order_id = @OrderId",
        new { eventData.OrderId, Now = DateTime.UtcNow },
        cancellationToken: ct);
}
```
