...
Example (ASP.NET Core minimal API or controller):
```csharp
// Minimal API
app.MapGet("/api/v1/orders/{orderId:guid}", async (Guid orderId, IOrderService svc) => await svc.GetByIdAsync(orderId));
app.MapPost("/api/v1/orders", async (CreateOrderRequest req, IOrderService svc) => Results.Created($"/api/v1/orders/{await svc.CreateAsync(req)}", null));
app.MapPut("/api/v1/orders/{orderId:guid}", async (Guid orderId, UpdateOrderRequest req, IOrderService svc) => await svc.UpdateAsync(orderId, req));
app.MapDelete("/api/v1/orders/{orderId:guid}", async (Guid orderId, IOrderService svc) => await svc.DeleteAsync(orderId));
app.MapPatch("/api/v1/orders/{orderId:guid}/status", async (Guid orderId, UpdateStatusRequest req, IOrderService svc) => await svc.UpdateStatusAsync(orderId, req));
```
...
Example Proto (same for all languages; C# client usage below):
```
...
C# gRPC client (generated from .proto):
```csharp
// Inject GrpcChannel and call
var client = new OrderService.OrderServiceClient(_grpcChannel);
var response = await client.GetOrderAsync(new OrderRequest { OrderId = orderId });
// Streaming
...
```csharp
// POST returns 202 Accepted with status URL
app.MapPost("/api/v1/orders", async (CreateOrderRequest req, IOrderProcessor processor) =>
{
    var requestId = Guid.NewGuid().ToString();
    _ = processor.ProcessAsync(requestId, req); // fire-and-forget or queue
    return Results.Accepted($"/api/v1/requests/{requestId}", new { requestId, statusUrl = $"/api/v1/requests/{requestId}" });
});

// GET status
app.MapGet("/api/v1/requests/{requestId}", async (string requestId, IRequestStatusStore store) =>
{
    var status = await store.GetAsync(requestId);
    return status == null ? Results.NotFound() : Results.Ok(status);
});
```

Response: 202 Accepted → `{ "requestId": "req-12345", "statusUrl": "/api/v1/requests/req-12345" }`  
GET status → 200 OK `{ "status": "completed", "result": { ... } }`  
Alternative: WebSocket or SignalR notification when ready
```
